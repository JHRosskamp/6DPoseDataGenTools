...
max_marker = 10
marker = bproc.loader.load_obj(filepath = os.path.join(args.bop_parent_path, 'ycbv/marker/marker.ply'))
#set scale here to m from mm
marker[0].set_scale([0.001, 0.001, 0.001])
marker[0].hide(True)
for i in range(max_marker-1):
    marker.append(marker[0].duplicate())
...
def sample_markers(obj: bproc.types.MeshObject, marker: list[bproc.types.MeshObject], int: max_marker):
    
    n_marker = np.random.randint(0, max_marker)

    sampled_markers = list(np.random.choice(marker, size=n_marker, replace=False))

    reset_markers(marker)

    for m in sampled_markers:
        tries = 0
        while check_marker_intersections(m.get_location()) or tries < 5:

            obj.edit_mode()
            bm = obj.mesh_as_bmesh()
            
            rot = obj.get_rotation_euler()
            index = np.random.randint(0, len(bm.faces)) 
            bm.faces.ensure_lookup_table()

            normal = bm.faces[index].normal
            center = bm.faces[index].calc_center_median_weighted()

            offset = center + 6 * normal #do this while in edit mode. otherwise normal is zero.
            obj.object_mode()

            offset_homogenous = np.ones(4)
            offset_homogenous[:3] = offset
            
            mw = obj.get_local2world_mat()
            m.set_location((mw@offset_homogenous)[:3]) 
            m.set_rotation_euler(rot) #not important because point.

            tries += 1

        m.hide(False)

# in: sampled marker position. compute distance to all other markers already in scene
def check_marker_intersections(position: np.ndarray) -> bool:
    
    #if position is at zero (not yet sampled)
    if np.linalg.norm(position) < 0.0001:
        return True

    threshold = 0.012
    counter = 0

    for m in marker: #for every marker except the on being sampled ie counter must be > 1
        m_pos = m.get_location()
        dist = np.linalg.norm(position-m_pos)
        if dist < threshold:
            counter += 1
    return True if counter > 1 else False

#
def reset_markers(marker: list[bproc.types.MeshObject]):
    for m in marker:
        m.hide(True)
        m.set_location([0.0, 0.0, 0.0])
...
for i in range(args.num_scenes):
    ...
    sample_markers(sampled_target_bop_objs[0], marker, max_marker)
